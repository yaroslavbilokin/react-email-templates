...
type ContentSchemaItem = TextContent | ButtonContent | HeadingContent | NoteBulletListContent | DividerContent | BasicBulletListContent | SignatureContent | MetricsContent;
...
const contentSchema: ContentSchemaItem[] = [
    ...
    {
        type: ContentType.text,
        content: "We've made excellent progress in reducing your cost per click from $10.80 to $9.60 in just the first month. Our ultimate goal is to bring this down to $5.00 or lower, which will significantly improve your marketing ROI.",
    },
    {
        type: ContentType.text,
        content: "It's important to note that we've tracked 11 phone calls directly from our campaign, but this number may be higher. Currently, we don't have access to track calls originating from your website, so there could be additional leads we're unable to attribute to our efforts.",
    },
    {
        type: ContentType.noteBulletList,
    ...
    {
        type: ContentType.text,
        content: "These tools will allow us to better track conversions, optimize your campaign, and provide more comprehensive reporting on your marketing performance.",
    },
    {
        type: ContentType.text,
        content: "We're looking forward to continuing our partnership and driving even better results in the coming months as we focus on increasing call volume and reducing acquisition costs.",
    },
    {
        type: ContentType.button,
    ...
    return contentSchema.map((item, index) => {
        switch (item.type) {
            case ContentType.heading:
                return <Heading key={index} style={h1}>{item.content}</Heading>;
            case ContentType.text:
                return <Text key={index}>{item.content}</Text>;
            case ContentType.button:
                return <Section style={buttonSection}>
                    <Button key={index} style={button} href={item.href} target="_blank">{item.content}</Button>
                </Section>;
            case ContentType.metrics:
                return (
                    <Section style={metricsContainer}>
                        {item.items.map((metricItem, metricIndex) => {
                            const {isImproved, showPercentageDiff, originalValue = null, currentValue, valuePrefix = null} = metricItem;
                            const isFirstItem = metricIndex === 0;
                            const isValueIncreased = currentValue > originalValue;
                            const percentageDiff = Math.abs(Math.floor(((currentValue - originalValue) / originalValue) * 100));

                            return (
                                <div key={metricIndex} style={{...metricsBox, ...(isFirstItem ? {marginTop: 0} : {})}}>
                                    <Text style={metricStatNumber}>
                                        {originalValue && <span style={isImproved ? metricImproved : metricWorsened}>{valuePrefix && valuePrefix}{`${formatNumber(originalValue)}`}</span>}
                                        <span>{valuePrefix && valuePrefix}{formatNumber(currentValue)}</span>
                                    </Text>
                                    <Text style={metricItemTitle}>
                                        <span>{metricItem.title}</span>
                                        {showPercentageDiff && <span style={isImproved ? percentageImproved : percentageWorsened}>{isValueIncreased ? '↑' : '↓'}{percentageDiff}%</span>}
                                    </Text>
                                </div>
                            )
                        })}
            ...
            case ContentType.noteBulletList:
                return (
                    <Section key={index} style={notesBulletListBox}>
                        <Heading as="h3" style={notesBulletListTitle}>{item.title}</Heading>
                        <ul style={notesBulletList}>
                            {item.items.map((bullet, bulletIndex) => (
                                <li style={notesBulletListItem} key={bulletIndex}>{bullet}</li>
                            ))}
                        </ul>
                    </Section>
                );
                ...
                return (
                    <Section key={index}>
                        ...
                        <ul style={basicBulletList}>
                            {item.items.map((bullet, bulletIndex) => (
                                <li style={basicBulletListItem} key={bulletIndex}>{bullet}</li>
                            ))}
                        </ul>
                    </Section>
                );
            ...
            case ContentType.signature:
                return (
                    <Section style={signatureSection} key={index}>
                        <Text style={signature}>{item.signOff}<br />{item.senderName}</Text>
                    </Section>
                );
            default:
                return null;
